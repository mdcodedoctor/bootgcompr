#' Wrapper function for easy setup of columns and analysis output when using the `median_diff_boot` function.
#'
#' @description
#' Function used in conjunction with `gtsummary` `tbl_summary()` and `bootgcompr` `median_diff_boot` function. Standard output when using `median_diff_boot` is not adjusted to the usual `tbl_summary()` output as a custom function is called. This wrapper function cleans up the outputted table and formats it as expected other types of `tbl_summary()` outputs, e.g. when using functions like `add_difference()`.
#'
...
#' library(bootgcompr)
#'
#' # build `tbl_summary()` with the `median_diff_boot` function
#' trial |>
#' tbl_summary(
#'  include = c(response),
#'  by = trt,
...
#'    pattern = "{stat} ({ci})") |>
#'  add_stat(
#'    fns = everything() ~ median_diff_boot(adj.vars = c("stage"), R = 1000)) |>
#'  median_diff_tbl()

# Easy wrapper function for modify_column_merge and modify_header
median_diff_tbl <- function(tbl_summary_obj,
